...
public final class ActivityThread {
    ...
    public static void loadClassAndInvoke(ClassLoader appClassloader, String eachclassname, Method dumpMethodCode_method) {
        ...
        try {
            //add-dc
            resultclass = Class.forName(eachclassname, true, appClassloader);//尝试这样是否可出发<clinit>函数的调用 //appClassloader.loadClass(eachclassname);
            //Log.i("ActivityThread", "" + eachclassname + " class is " + resultclass.toString()); 
            //add-dc   
        }catch (ClassNotFoundException e)
            e.printStackTrace();
            return;
        ...
        if (resultclass != null) {
            try {
                ...
                if (resultclass.isInterface()){
                    Log.i("ActivityThread", eachclassname + " is a interface, can't call, or the app will crashes.");
                    return ;
                }
                //add-dc
    ...
    private static String getClassFileContent(String strFileName){
        try{
    ...
    public static void fart() {
        ...
        for (int j = 0; j < ElementsArray.length; j++) {
            ...
            if (dexfile != null) {
                ...
                //     }
                // }
                 //fix to read classname from file
                //add-dc
                try {
                    String strClassNames = getClassFileContent("/sdcard/class_list.txt");
                    if (strClassNames == null){
                        Log.v("ActivityThread", "String strClassNames = getClassFileContent(\"/sdcard/class_list.txt\"); return is null");
                        return ;
                    }
                    else{
                        String[] aryClassAry = strClassNames.split("\n");
                        int index = 0;
                        for (String str : aryClassAry){
                            String clsName = str.trim().replace("/", ".");
                            clsName = clsName.substring(1, clsName.length()-1);
                            try{
                                Log.v("ActivityThread", "NO." + (++index) + ":go into loadClassAndInvoke->" + "classname:" + clsName);
                                loadClassAndInvoke(appClassloader, clsName, dumpMethodCode_method);
                            }
                            catch(Exception e){
                                Log.v("ActivityThread", "loadClassAndInvoke " + clsName + " " + e.toString());
                            }
                        }
                    }
                }catch(Exception e){
  ...
  //add
    ...
    private void sendMessage(int what, Object obj, int arg1, int arg2, boolean async) {
     ...
     //add-dc
     private static boolean mIsRunFartThread = false;
     //add-dc
    ...
    private Activity performLaunchActivity(ActivityClientRecord r, Intent customIntent) {
        ...
        try {
            ...
            if (localLOGV) Slog.v(
             ...
             //add-dc
             dc_packageName = app.getPackageName();
             //add-dc
            if (activity != null) {
         ...
         //add-dc
         if (dc_packageName != null && dc_packageName.equals("com.example.test") && !mIsRunFartThread){ //Just pull shell for one app
            mIsRunFartThread = true;
        //add-dc
            fartthread();
        }
		//add
