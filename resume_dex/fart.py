...
class method_code:
	...
	#add
	def replaceMethodIns(self,dex_object,bytearray_str):
		bytes = dex_object.m_content[self.insns:self.insns + self.insns_size * 2]
		print "pre ins length:{}, bytearray_str length:{}".format(len(bytes), len(bytearray_str))
...
class dex_encode_method:
	...
	def printf_l(self,dex_object,is_virtual):
		...
		if self.m_code_off!=0:	
			self.m_code_item.printf(dex_object)
class dex_class:
	def __init__(self,dex_object,classid,dex_byte_list):# dex_file_content 加在这里是为了修改其中内容，然后写到新的文件中
		if classid >= dex_object.m_classDefSize:
			return ""
		self.m_dex_byte_list = dex_byte_list
	...
	def replace_bytes(self, code_off, new_bytes, length):
		if code_off + length >= len(self.m_dex_byte_list):
			return
		for i in range(length):# 暂时用最笨的办法
			self.m_dex_byte_list[code_off+i] = new_bytes[i]
			...
			if code_off != 0:
				...
				if method != None:
					...
					try:
						...
						#bytes = self.content[self.insns:self.insns + self.insns_size * 2]
						method_code_obj = method_code(dex_object, code_off)
						method_code_obj.printf(dex_object, "\t\t")
					except Exception as e:
						print e
					try:
						...
						print e
					try:
						self.replace_bytes(code_off, bytearray_str, len(bytearray_str))
					except Exception as e:
						print e
			...
			if code_off != 0:
				...
				if method != None:
					...
					try:
						...
						method_code_obj2 = method_code(dex_object, code_off)
						method_code_obj2.printf(dex_object, "\t\t")
					except Exception as e:
						print e
					try:
						bytearray_str = base64.b64decode(method.insarray)
...
class dex_parser:
	...
	def __init__(self,filename):
		...
		for i in xrange(0,self.m_classDefSize):
			str1 = self.getclassname(i)
			dex_class(self,i,self.m_dex_byte_list).printf(self)
			pass
			#self.getclass(i)
		self.write_new_dex()

	def write_new_dex(self):
		with open(self.m_new_dex, "wb") as fw:
			fw.write(''.join(self.m_dex_byte_list))
